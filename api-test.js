...
const cacheTopResponse = require(path.join(__dirname, 'modules', 'cacheTop.js'));
 ...
 */
// function sortUsersByKDR(top, users, weapons, bannedPlayers) {
//   var arr = [];
...
function roundEnd() {
  updated = true;
  cacheTopResponse(top, users, weapons, bannedPlayers, totalPlayers, lastUpdate, updated).then(cacheDemos).then(demos => {
    demoList = demos;
  }).catch(console.error);
}
